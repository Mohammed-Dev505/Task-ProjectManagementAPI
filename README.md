...
All input DTOs are validated using **FluentValidation** with custom error messages.

| DTO | Validation Rules |
|-----|-----------------|
| `RegisterModel` | Username (3–50 chars), valid Email, Password (min 6 chars) |
| `TokenRequestModel` | Valid Email, Password required |
| `CreateProjectDto` | Name required (3–150 chars), Description max 500 chars |
| `UpdateProjectDto` | Valid ID, Name required |
| `CreateTaskDto` | Title required (3–100 chars), ProjectId > 0, DueDate in future |
| `UpdateTaskDto` | Valid ID, Title required, DueDate in future |
| `UpdateTaskStatusDto` | Valid ID, valid Status enum value |
